...
class TestOptions():
    def __init__(self):
        ...
        self.parser.add_argument("--style_id", type=int, nargs='+', default=[9], help="a series of ids of the style images")
        self.parser.add_argument("--split_time", type=float, nargs='+', default=None, help="(optional) list of id switching times")
...
if __name__ == "__main__":
    ...
    if args.backbone == 'dualstylegan':
        ...
        exstyles = [torch.tensor(exstyles[list(exstyles.keys())[s]]).to(device) for s in args.style_id]
        with torch.no_grad():  
    ...
    if args.video:
        ...
        for i in tqdm(range(num)):
            ...
            with torch.no_grad():
                ...
                if len(exstyles) == 1:
                    # no interpolation
                    cur_exstyle = exstyles[0]
                elif args.split_time is None:
                    if i == num - 1:
                        sid = len(exstyles) - 2
                    else:
                        sid = i * (len(exstyles) - 1) // (num - 1)
                    ratio = i * (len(exstyles) - 1) / (num - 1) - sid
                else:
                    split_time = [0.] + args.split_time + [1.]
                    assert len(split_time) == len(exstyles)
                    sid = 0
                    ratio = i / (num - 1)
                    while sid < len(split_time) and ratio > split_time[sid + 1]:
                        sid += 1
                    ratio = (ratio - split_time[sid]) / (split_time[sid + 1] - split_time[sid])
                cur_exstyle = exstyles[sid + 1] * ratio + exstyles[sid] * (1 - ratio)
